...
object EfficientAndroidLWJGLKeycode {
    ...
    fun generateKeyName(): Array<String?> {
        if (androidKeyNameArray == null) {
            androidKeyNameArray = arrayOfNulls<String>(sAndroidKeycodes.size)
            for (i in androidKeyNameArray!!.indices) {
                androidKeyNameArray!![i] =
    ...
    fun execKey(keyEvent: KeyEvent, valueIndex: Int) {
        ...
        println(keyEvent.getKeyCode().toString() + " " + keyEvent.getDisplayLabel())
        val key =
            (if (keyEvent.getUnicodeChar() != 0) keyEvent.getUnicodeChar() else '\u0000'.code).toChar()
        sendKeyPress(
    ...
    @JvmStatic
    fun getValueByIndex(index: Int): Short {
        return sLwjglKeycodes[index]
    }

    fun getIndexByKey(key: Int): Int {
    ...
    private fun add(androidKeycode: Int, LWJGLKeycode: Short) {
